      ...
      transfer : (from_principal, from_sa, to_user, amount, fee, memo, notify) => {
        return new Promise((resolve, reject) => {
          ...
          switch(tokenObj.canister) {
            case LEDGER_CANISTER_ID:
              args = {
                "from_subaccount" : [getSubAccountArray(from_sa ? from_sa : 0)], 
            ...
            default:
              args = {
                'token' : tid,
                'from' : { 'address' : principalToAccountIdentifier(from_principal, from_sa ? from_sa : 0) },
                'subaccount' : [getSubAccountArray(from_sa ? from_sa : 0)],
...
const extjs = {
  connect : (host, identity) => new ExtConnection(host ? host : "https://boundary.ic0.app/", identity),
